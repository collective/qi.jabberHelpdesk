...
class IHelpdesk(Interface):
    ...
    botJid = schema.ASCIILine(title=_(u"Bot's JID"),
                            description=_(u"The Jabber ID of the bot assigned to this helpdesk"),
                            constraint = email_constraint,
                            required=True)
    botPassword = schema.Password(title=_(u"Bot's password"),
                            description=_(u"The bot's password."),
                            default=None,
                            required=False)
    
    persistent = schema.Bool(title=_(u"Persistent bot"),
                            description = _(u"Specifies whether the bot remains loaded even if nobody is using it"),
                            default = False,
                            )
class IHelpdeskAgents(Interface):
    """
    """
    agentJids = schema.List(title = _(u"Agents IDs"),
                            description = _(u"The jabber IDs of auhorized agents, typically of the form userID@jabberserver"),
